...
export class ListUserMessageComponent implements OnInit {
  ...
  showMsgList2(user : User){
    ...
    this.userMessage = user ;
    if(this.isProfessionnel)
      this.messageService.updateVuUser(user.id,this.user.id,user.id).subscribe((data) => {});
    else
      this.messageService.updateVuUser(this.user.id,user.id,user.id).subscribe((data) => {});  
  ...
  getListProfMessage(){
    ...
    this.messageService.getAllMessage().subscribe((data) => {
      ...
      this.listeUser.forEach((ele) => {
        this.messageService.getUserViewNbr(this.user.id,ele.id,ele.id).subscribe(data => {
  ...
  getListDemandMessage(){
    ...
    this.messageService.getAllMessageFromDemandeur().subscribe((data) => {
      ...
      let i = 0 ;
      this.listeUser.forEach((ele) => {
        this.messageService.getUserViewNbr(ele.id,this.user.id,ele.id).subscribe(data => {
            let mg : MessageVu = {nbrvu: '0' , userInfo: this.user} ;
            //this.messageVu = data;
            let messageVis: MessageVu[] = data;
            mg.userInfo = ele
